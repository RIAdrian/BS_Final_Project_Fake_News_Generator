...
def username_exists(username, db):
    conn = sqlite3.connect(db)  # creaza o conexiune la baza de date specificată prin calea db folosind modulul sqlite3
    cursor = conn.cursor() #  Un cursor este un obiect care permite executarea comenzilor SQL și navigarea rezultatelor interogărilor
    cursor.execute('SELECT 1 FROM users WHERE username = ?', (username,)) # verifica dacă există un rând în tabelul users unde coloana username este egală cu valoarea username
    #Dacă interogarea nu returnează niciun rând (adică numele de utilizator nu există în baza de date), fetchone() va returna None
    #Prin urmare, expresia cursor.fetchone() is not None va fi True dacă un rând a fost găsit și False dacă nu
...
def authenticate_user(username, password, db):
    ...
    #indică faptul că numele de utilizator nu a fost găsit în baza de date. 
    #in acest caz, funcția returnează un tuplu care conține False (indicând că autentificarea a eșuat), un mesaj de eroare "Username-ul nu există."
    elif result[1] != password:
        return False, "Parola este incorectă.", None, None
    #verifică dacă parola furnizată (password) nu se potrivește cu parola stocată în baza de date (result[1]). 
    #dacă parolele nu se potrivesc, funcția returnează un tuplu care conține False
    return True, None, result[0], result[2] 
    #Dacă numele de utilizator a fost găsit și parola furnizată se potrivește cu parola stocată în baza de date, funcția returnează un tuplu care conține True

# Funcție pentru a verifica datele pentru resetarea parolei
def validate_reset_password(username, email, current_password, db):
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
...
# Funcție pentru resetare parola
...
# Funcție pentru creare cont nou
def create_account(email, username, password, date_of_birth, db):
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
    try:
...
def init_db(db):
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
    # Constrangeri/ restrictii baza de date 
    cursor.execute('''
...
# Apel de creare baza de date 
init_db('users_admin.db')
init_db('users_client.db')

# Încărcare model GPT-2
@st.cache_resource # memoreaza rezultatele functiilor astfel incat sa economiseasca timp prin a nu mai rula functia respectiva din nou 
...
# sunt functii folosite în procesarea limbajului natural (NLP) pentru a împărți textul în bucăți mai mici, numite tokeni.
tokenizer.pad_token = tokenizer.eos_token
tokenizer_train.pad_token = tokenizer_train.eos_token

# Funcție pentru generare text
...
# Functie folosita in app admin - antrenare 
def count_texts_in_db(db):
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
...
def train_model(model, tokenizer, texts):
    # pentru antrenarea din app o sa folosesc o metoda de fine tuning 
    optimizer = AdamW(model.parameters(), lr=5e-5)
    total_steps = len(texts) * 3  
...
def get_random_texts(df, num_texts=10):
    return df.sample(n=num_texts).reset_index(drop=True) # returnează un subset de df care conține num_texts rânduri selectate aleatoriu
...
def generate_report(questions, user_answers, correct_answers, username, email, file_path):
    with open(file_path, mode='w', newline='', encoding='utf-8') as file:
        ...
        writer.writerow(["Data finalizarii", datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
